...
export const emptyBrain = (): BrainSlice => ({
  facts: [], voice: [], citations: [], gaps: [], commitments: [], limits: [],
});
...
export function sliceBrain(brain: BrainSlice, needs: AgentNeeds): BrainSlice {
  return {
    facts: needs.factKeys?.length
      ? brain.facts.filter((f) => needs.factKeys!.some((k) => f.key === k || f.key.startsWith(`${k}.`)))
      : brain.facts,
...
export function renderBrain(brain: BrainSlice): string {
  ...
  const partes: string[] = [];
  if (verdes.length) {
    partes.push(`HECHOS CONFIRMADOS (los únicos citables):\n${verdes.map((f) => `- ${f.key}: ${f.value}`).join("\n")}`);
  }
  const amarillos = brain.facts.filter((f) => f.status !== "verde");
  if (amarillos.length) {
    partes.push(`NO CONFIRMADOS (no los uses como dato duro):\n${amarillos.map((f) => `- ${f.key}`).join("\n")}`);
  }
  if (brain.voice.length) {
    partes.push(`VOZ TEXTUAL (únicas frases que puedes entrecomillar como suyas):\n${brain.voice.map((v) => `- "${v.text}"`).join("\n")}`);
  }
  if (brain.commitments.length) {
    partes.push(`COMPROMISOS:\n${brain.commitments.map((c) => `- ${c.promise}${c.dueDate ? ` (${c.dueDate})` : ""}`).join("\n")}`);
  }
  if (brain.limits?.length) {
    partes.push(`LÍMITES NARRATIVOS (no los abordes):\n${brain.limits.filter((l) => l.level !== "publico").map((l) => `- ${l.topic}`).join("\n")}`);
  }
  return partes.join("\n\n");
}
