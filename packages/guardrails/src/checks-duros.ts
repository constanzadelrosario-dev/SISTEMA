...
export function chequeoValidacion(texto: string, ctx: GuardContext): CheckResult {
  ...
  for (const termino of TERMINOS_EFICACIA) {
    for (const pos of findWord(texto, termino)) {
      ...
      spans.push({
        start: pos,
        end: pos + termino.length,
        text: texto.slice(pos, pos + termino.length),
      });
    }
  }
...
export function chequeoVerbatim(texto: string, ctx: GuardContext): CheckResult {
  ...
  if (citas.length === 0) {
    return {
      ...
      passed: true,
      severity: "bloqueante",
      detail: "No hay citas que verificar.",
    };
  }
  ...
  for (const cita of citas) {
    ...
    if (existe) continue;
    (externa ? sinRegistrar : inventadas).push({
      start: cita.start,
      end: cita.end,
      text: cita.text,
    });
...
export function chequeoCoherencia(texto: string, ctx: GuardContext): CheckResult {
  ...
  const sospechosas: Span[] = fechas
    .map((m) => ({ m, t: m[1] ?? "" }))
    .filter(({ t }) => !conocidas.some((f) => f.includes(t) || t.includes(f)))
    .map(({ m, t }) => ({ start: m.index ?? 0, end: (m.index ?? 0) + t.length, text: t }));
  ...
  if (sospechosas.length === 0) {
    return {
      ...
      passed: true,
      severity: "advertencia",
      detail: "Sin fechas en conflicto.",
    };
  }
  return {
    id: "coherencia",
...
export function chequeoLimites(texto: string, ctx: GuardContext): CheckResult {
  ...
  if (spans.length === 0) {
    return {
      ...
      passed: true,
      severity: "advertencia",
      detail: "No cruza límites declarados.",
    };
  }
  return {
    id: "limites",
