...
async function juzgar(
  ...
  try {
    ...
    return { id, passed: data.passed, severity, detail: data.detail, suggestions: data.suggestions };
  } catch (err) {
    ...
    return {
      id, passed: true, severity: "advertencia",
...
Devuelve SOLO JSON: { "passed": bool, "detail": "…", "suggestions": ["…"] }`,
    texto, ctx.domain === "salud_mental" ? "bloqueante" : "advertencia");
