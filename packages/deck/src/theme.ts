...
export function generarAlternativas(
  ...
  const out: Array<{
    name: string;
    origin: "extraido" | "variacion" | "contraste";
    tokens: ThemeTokens;
  ...
  for (const eje of ejes.slice(0, 2)) {
    ...
    if (eje === "fonts") {
      ...
      t.density = {
        padY: Math.round(t.density.padY * 0.7),
        padX: Math.round(t.density.padX * 0.7),
        padXLg: Math.round(t.density.padXLg * 0.7),
        lineHeight: 1.35,
      };
  ...
  out.push({
    name: claro ? "Contraste claro" : "Contraste oscuro",
    origin: "contraste",
    tokens: contraste,
  });
...
export const themeToCss = (t: ThemeTokens): string =>
  [
    ...
    `--f-title:${t.fonts.title}`,
    `--f-body:${t.fonts.body}`,
    `--f-ui:${t.fonts.ui}`,
    `--h1:${t.scale.h1}px`,
    `--h2:${t.scale.h2}px`,
    `--body:${t.scale.body}px`,
    `--pad-y:${t.density.padY}px`,
    `--pad-x:${t.density.padX}px`,
    `--pad-x-lg:${t.density.padXLg}px`,
    `--lh:${t.density.lineHeight}`,
