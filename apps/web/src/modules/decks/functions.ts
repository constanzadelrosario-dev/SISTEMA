...
import { DECK_EJEMPLO, Deck, generarAlternativas, TEMA_BASE } from "@sistema/deck";
...
export const fnCreateDeck = createServerFn({ method: "POST" })
  .middleware([requireAuth])
  .validator((d: { deck?: unknown; subtype?: string }) =>
  ...
  .handler(async ({ data, context }) => {
    ...
    return createDeck(context.sb, context.workspaceId, data.deck ?? DECK_EJEMPLO, {
...
export const fnSaveDeck = createServerFn({ method: "POST" })
  .middleware([requireAuth])
  .validator((d: { id: string; deck: unknown }) =>
...
export const fnGenerarTemas = createServerFn({ method: "POST" })
  .middleware([requireAuth])
  .validator((d: { baseThemeId?: string }) =>
